...
For my second milestone, I finished the whole robot. I added the remote, the WLAN module, and the transceiver wireless module. What the wireless module does is that there is 1 on the remote, and 1 on the arduino. The one on the remote sends signals to the arduino one which makes the motors move with it. Some challenges I faced when doing this was that my remote could connect to everyone elses exept mine. This was because the address for all of our robots were the same. robot.setRemote(byte byte0, ....,byte byte04) and then set the remote to the same address remote.Set(byte byte0, ...., byte byte05).
